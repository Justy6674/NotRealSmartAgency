...
- **Publishing** — `src/lib/publishers/dispatcher.ts` `publishToPlatform()`. The door to a live account, and the only place the transport is chosen: **Zernio** when the brand has a `zernio_profile_id` in `brands.social_urls` *and* a Zernio account whose platform matches exactly (subscribers), else native (`USE_NATIVE_PUBLISHER_*`), else self-hosted **Mixpost** — which is what Justin's own brands run on, so it stays exercised daily and is a genuine fallback rather than a dusty one. Write new publishing code against it; never call `createZernioPost` or `createMixpostPost` from a route.
  ...
  **Write path, re-checked 2026-08-17 after `f2d9aac` / `bd1d307`:** cron, `/api/scheduled-posts/publish-now`, and `publish_to_social` all call `publishToPlatform`. Confirm with `grep -rln "publishToPlatform" src` — those three plus `dispatcher.ts` and the invariant tests must appear. An earlier line in this file said only the cron did; that was true in the morning and false by evening. **The remaining gap is reads:** social analytics, Mixpost account routes, and the performance learner still call `fetchMixpostAccounts`. `/api/studio/overview` now uses Zernio when `brands.social_urls.zernio_profile_id` is set (`src/lib/studio/overview-accounts.ts`), Mixpost otherwise. This is not a Build-First violation: NRS still owns brand intelligence, the AHPRA/TGA gate, the composer, scheduling and memory; Zernio is a transport behind our interface. See `2026-08-17-nrs-zernio-for-subscribers-mixpost-as-fallback.md`.
