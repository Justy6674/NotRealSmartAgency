...
/**
 * Mastodon's light theme: white, #c0cdd9 borders, #606984 handles, and the
 * #6364ff purple the logo carries.
 *
 * This is the one call in the set worth writing down, because a reader who
 * knows Mastodon will think it is a mistake. Mastodon's own WEB client does
 * default to its dark theme — so "keep it dark" is a defensible reading. Light
 * wins here for two reasons. The official apps follow the device, which is
 * overwhelmingly light; and this pane's job is to answer one question — does
 * the post look right — for which the useful answer is the one most readers
 * will see, not the one mastodon.social happens to ship as its default skin.
 * Mastodon's light theme is a real, first-class Mastodon theme, so this is a
 * choice between two true pictures rather than a fiction. If the owner ever
 * says otherwise, swap FEED, LINE and INK for the dark set (#191b22 / #282c37 /
 * #d9e1e8) — do not repaint it half-way.
 */
const FEED = 'oklch(1 0 0)'
const WASH = 'oklch(0.968 0.004 236)'
const LINE = 'oklch(0.842 0.022 246)'
const INK = 'oklch(0.15 0 0)'
const INK_2 = 'oklch(0.523 0.044 270)'
const PURPLE = 'oklch(0.593 0.224 277)'
        ...
        <div className="flex items-center justify-center px-3 py-2" style={{ borderBottom: `1px solid ${LINE}` }}>
          <svg width="20" height="20" viewBox="0 0 24 24" fill="none">
            <path
              ...
              fill={PURPLE}
            />
            {/* The 'm' is a cut-out, so it is painted in the ground colour. */}
            <path
          ...
          <div
            ...
            style={{
              ...
              background: brandAvatarUrl
                ...
                : 'linear-gradient(135deg, oklch(0.593 0.224 277), oklch(0.68 0.18 300))',
            }}
          />
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-1 min-w-0">
                ...
                <span className="truncate" style={{ fontSize: 11, color: INK_2 }}>@{handle}@mastodon.social</span>
              </div>
              <MoreHorizontal className="h-3.5 w-3.5 flex-shrink-0" style={{ color: INK_2 }} />
            </div>
