...
5. On approval → call draft_post with the finalised caption + hashtags + media_id (for single-media) to land it in Review

NEVER write captions or hashtags yourself. NEVER skip propose_post_from_media and call draft_post directly. NEVER finalise without explicit user approval. The AI client (Claude/Grok/Gemini via MCP) is acting on behalf of a human user — wait for their sign-off before writing to scheduled_posts.`
