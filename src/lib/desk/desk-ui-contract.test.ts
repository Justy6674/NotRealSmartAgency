...
const composeScreen = readFileSync(resolve(process.cwd(), 'src/components/agency/studio/post/ComposeScreen.tsx'), 'utf8')
...
test('Creator and Review restore exact Desk receipts after navigation or refresh', () => {
  assert.match(composeScreen, /searchParams\.get\('draft'\)/)
  assert.match(composeScreen, /searchParams\.get\('media'\)/)
  assert.match(composeScreen, /searchParams\.get\('conversation'\)/)
  assert.match(composeScreen, /searchParams\.get\('output'\)/)
  assert.match(composeScreen, /deskConversationId/)
  assert.match(composeScreen, /deskOutputId/)
